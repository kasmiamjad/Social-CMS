...
/**
 * Client-side reply input box for an Instagram DM conversation. Sends via
 * /api/v1/instagram/conversations/:id/messages and refreshes the thread's
...
export function ManualReplyInput({ conversationId }: ManualReplyInputProps) {
  ...
  async function handleSend() {
    ...
    try {
      const res = await fetch(`/api/v1/instagram/conversations/${conversationId}/messages`, {
        method: "POST",
  ...
  return (
    <div className="mt-6 pt-4 border-t border-border">
      ...
      <p className="mt-1.5 text-[10px] text-text-muted">
        Free-form messages work only within Instagram's 24-hour standard messaging window.
      </p>
    </div>
  );
}
