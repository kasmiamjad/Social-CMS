...
function gradientId(status: string): string {
          ...
          <ComposedChart data={points} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
            <defs>
              {A360_STATUS_VALUES.map((status) => (
                <linearGradient key={status} id={gradientId(status)} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={A360_STATUS_HEX[status]} stopOpacity={0.45} />
                  <stop offset="100%" stopColor={A360_STATUS_HEX[status]} stopOpacity={0.02} />
                </linearGradient>
              ))}
            </defs>
            ...
            />
            {/* One gradient-filled area per status, drawn first so every Line renders on top. */}
            {A360_STATUS_VALUES.map((status) => (
              <Area
                key={status}
                type="monotone"
                dataKey={status}
                fill={`url(#${gradientId(status)})`}
                stroke="none"
                legendType="none"
